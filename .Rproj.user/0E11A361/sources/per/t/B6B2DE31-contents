...
ES = filter(Data, Species == "Pityopsis trayci")
summary(ES)
...
############################### 2022 Data ######################################
# Check Assumptions #
model  <- lm(Coverage ~ Treatment, data = ES_22)
...
ES_22$Treatment= as.factor(ES_22$Treatment)
ES_22 %>% levene_test(Coverage ~ Treatment)

# Test for Significance #
anova_22 = ES_22 %>% anova_test(Coverage ~ Treatment) %>% 
  add_significance()
summary(anova_22)
...
############################### 2023 Data ######################################
# Check Assumptions #
model  <- lm(Coverage ~ Treatment, data = ES_23)
# Create a QQ plot of residuals
ggqqplot(residuals(model))
# Compute Shapiro-Wilk test of normality
shapiro_test(residuals(model))
plot(model, 1)
# Compute Levene's Test
ES_23$Treatment= as.factor(ES_23$Treatment)
ES_23 %>% levene_test(Coverage ~ Treatment)

# Test for Significance #
anova_23 = ES_23 %>% anova_test(Coverage ~ Treatment) %>% 
  add_significance()
summary(anova_23)
...
ESBox22 = 
  ggplot(ES_22, aes(x = Treatment, y = Coverage), colour = Treatment) +
